const fs = require("fs-extra");
                                                                       ...
                                                                       const spamPath = `${process.cwd()}/spamData.json`;

// ===== SPAM CACHE (FUTURE SAFE) =====
global.__spamCache = global.__spamCache || {
  userRate: {},
  groupRate: {},
  threads: {},
  users: {},
...
if (!global.__spamFlushInterval) {
  global.__spamFlushInterval = setInterval(() => {
    if (!global.__spamCache.dirty) return;
    try {
      fs.writeJsonSync(spamPath, global.__spamCache, { spaces: 2 });
      global.__spamCache.dirty = false;
    } catch {}
  }, 15000);
}

function spamProtect(api, threadID, senderID, commandName) {
  const now = Date.now();
  const data = global.__spamCache;

  data.userRate[senderID] = (data.userRate[senderID] || []).filter(t => now - t < 10000);
  data.userRate[senderID].push(now);

  data.groupRate[threadID] = (data.groupRate[threadID] || []).filter(t => now - t < 300000);
  data.groupRate[threadID].push(now);

  data.users[senderID] = (data.users[senderID] || 0) + 1;
  data.threads[threadID] = (data.threads[threadID] || 0) + 1;
  data.commands[commandName] = (data.commands[commandName] || 0) + 1;

  if (data.userRate[senderID].length >= 8) {
    try {
      global.db.allThreadData.find(t => t.threadID == threadID)?.data?.muted?.push(senderID);
      api.sendMessage("🛡️ Auto spam detect: plz off", threadID);
    } catch {}
  }

  if (data.groupRate[threadID].length >= 200) {
    try {
      api.sendMessage("📴 Too much spam, bot leaving group", threadID, () => {
...
function logSpam(threadID, senderID, commandName) {

  if (!fs.existsSync(spamPath)) {

    fs.writeJsonSync(spamPath,{threads:{},users:{},commands:{}},{spaces:2});

  }

  const data = fs.readJsonSync(spamPath);
  ...
  fs.writeJsonSync(spamPath,data,{spaces:2});
...
module.exports = function (api, threadModel, userModel, dashBoardModel, globalModel, usersData, threadsData, dashBoardData, globalData) {
        ...
        const rawCommandName =
          body && body.startsWith(prefix)
            ? body.slice(prefix.length).trim().split(/ +/)[0]?.toLowerCase()
            : "__chat__";

        spamProtect(api, threadID, senderID, rawCommandName || "__chat__");

                        // 🛡️ AUTO SPAM PROTECTION (GLOBAL)
                        // —————————————— CHECK USE BOT —————————————— //
