// HandlerEvent customized version by Rakib.
...
async function spamProtect(
  ...
  data.userRate[senderID] =
    (data.userRate[senderID] || []).filter(
      t => now - t < 10000
    );

  data.userRate[senderID].push(now);

  data.groupRate[threadID] =
    (data.groupRate[threadID] || []).filter(
      t => now - t < 300000
    );
  ...
  data.users[senderID] =
    (data.users[senderID] || 0) + 1;

  data.threads[threadID] =
    (data.threads[threadID] || 0) + 1;

  data.commands[commandName] =
    (data.commands[commandName] || 0) + 1;
  ...
  if (data.userRate[senderID].length >= 8) {
    try {
      ...
      if (!Array.isArray(thread.data.muted))
        thread.data.muted = [];

      if (!thread.data.muted.includes(senderID)) {
        thread.data.muted.push(senderID);
      ...
      api.sendMessage(
        "🛡️ Auto spam detected. Please off.",
        threadID
      );
    }
    catch (e) {
  ...
  // GROUP SPAM
  if (data.groupRate[threadID].length >= 500) {
    try {
      api.sendMessage(
