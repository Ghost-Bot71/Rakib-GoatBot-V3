...
module.exports = {
  config: {
    ...
    countDown: 5,
    role: 0,
    shortDescription: "Give a funny kick",
    category: "fun",
  ...
  onStart: async function ({ message, event, usersData }) {
    ...
    if (!targetID)
      return message.reply("🦶 কাউকে reply বা mention করো উষ্টা দেওয়ার জন্য!");
    ...
    try {
      ...
      // Background
      const bgURL =
        "https://drive.google.com/uc?export=download&id=1PjSRUG7kTXxy0taFUxJqTMYjvf069PeS";
      ...
      const imgPath = path.join(
        tmpDir,
        `usta_${senderID}_${targetID}_${Date.now()}.png`
      );

      await fs.writeFile(imgPath, canvas.toBuffer("image/png"));

      // Random usta captions
      const captions = [
        `🦶 ${name1} জোরে একটা উষ্টা মারলো ${name2} কে!`,
        `💥 ${name2} খেয়ে ফেললো ${name1} এর legendary উষ্টা!`,
        `🤣 ${name1} দিলো এমন উষ্টা যে ${name2} উড়ে গেল!`,
        `🔥 ${name1} vs ${name2} — উষ্টা যুদ্ধ শুরু!`,
        `😆 ${name2} আজ ${name1} এর উষ্টার শিকার!`,
        `💢 ${name1} বললো: এইটা নে উষ্টা! ${name2}`
      ];
      ...
      message.reply("⚠️ Usta render failed!");
    }
  }
};
