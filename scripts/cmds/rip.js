...
module.exports = {
  ...
  onStart: async function ({ message, event, usersData }) {
    ...
    if (!targetID)
      return message.reply("⚰️ কাউকে reply বা mention করো।");
    ...
    try {
      ...
      if (!avatarUrl) return message.reply("❌ Avatar পাওয়া যায়নি।");

      // Background
      const bgURL =
        "https://drive.google.com/uc?export=download&id=17PoH-_3t0_qWQ2rU_DZ-eM4eUq5aqNNh";
      ...
      // random RIP captions
      const captions = [
        `⚰️ Rest in peace ${name}`,
        `🪦 ${name} will be remembered...`,
        `💀 ${name} didn't survive the moment`,
        `🥀 Farewell ${name}`,
        `🕯️ In memory of ${name}`,
        `⚰️ ${name} is gone but not forgotten`
      ];
      ...
      message.reply("⚠️ RIP render failed!");
    }
  }
};
