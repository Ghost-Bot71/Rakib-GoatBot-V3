...
const ACTIVE_ISLAMIC = new Map();

module.exports = {
  config: {
    name: "islamic",
    aliases: ["iqz", "islamicquiz"],
    version: "FINAL-EDIT",
    author: "Rakib",
    role: 0,
    category: "game",
    guide: {
      en: "islamic → get islamic quiz\nReply A / B / C / D"
    }
  },
  ...
  onStart: async function ({ message, event, api, usersData }) {
    ...
    if (ACTIVE_ISLAMIC.has(uid)) {
      return message.reply("⚠️ তুমি ইতিমধ্যে একটি Islamic Quiz খেলছো!");
    }

    try {
      const user = await usersData.get(uid) || {};
      const token = user.data?.islamicToken || "";

      const res = await axios.get(
        `https://rakib-api.vercel.app/api/quiz?category=Islamic&apikey=rakib69&token=${token}`
      );
      ...
      if (!["A", "B", "C", "D"].includes(answer)) {
        return message.reply("❌ Islamic Quiz data invalid!");
      }
...
${answer}) ${q[answer]}`,
      ...
      ACTIVE_ISLAMIC.delete(uid);
      message.reply("❌ Islamic Quiz লোড করা যাচ্ছে না!");
    }
  },
  ...
  onReply: async function ({ message, event, usersData, Reply, api }) {
    ...
    let win = data.islamicWin || 0;
    let loss = data.islamicLoss || 0;
    let streak = data.islamicStreak || 0;
    let bestStreak = data.islamicBestStreak || 0;
    let badges = data.islamicBadges || [];
    ...
    if (ans === correct) {
      ...
      await usersData.set(uid, {
        ...
        data: {
          ...
          islamicWin: win,
          islamicLoss: loss,
          islamicStreak: streak,
          islamicBestStreak: bestStreak,
          islamicBadges: badges,
          islamicToken: Reply.token
        }
      });
