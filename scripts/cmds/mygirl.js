...
module.exports = {
  config: {
    ...
    countDown: 5,
    role: 0,
    shortDescription: "My girl moment",
    category: "fun",
  ...
  onStart: async function ({ message, event, usersData }) {
    ...
    if (!targetID)
      return message.reply("💖 কাউকে reply বা mention করো, এইটা তোমার girl!");
    ...
    try {
      ...
      // Background image
      const bgURL =
        "https://drive.google.com/uc?export=download&id=15qPfiKCocynsz7H0NEjha6ViJvecC2Zv";
      ...
      const imgPath = path.join(
        tmpDir,
        `mygirl_${senderID}_${targetID}_${Date.now()}.png`
      );
      ...
      // Random captions
      const captions = [
        `💖 ${name1} বলছে — এইটা আমার girl ${name2}!`,
        `😍 ${name2} is officially ${name1}'s girl!`,
        `💘 ${name1} & ${name2} — perfect duo!`,
        `✨ ${name1} proudly saying: that's my girl ${name2}!`,
        `💕 ${name2} always special for ${name1}!`,
        `💞 ${name1} and ${name2} — cute couple vibes!`
      ];
      ...
      console.error("MyGirl error:", err);
      message.reply("⚠️ MyGirl render failed!");
    }
  }
};
