...
module.exports = {
  config: {
    ...
    countDown: 5,
    role: 0,
    shortDescription: "My king moment",
    category: "fun",
  ...
  onStart: async function ({ message, event, usersData }) {
    ...
    if (!targetID)
      return message.reply("👑 কাউকে reply বা mention করো, এটাই তোমার king!");
    ...
    try {
      ...
      // Background image
      const bgURL =
        "https://drive.google.com/uc?export=download&id=1lDbXYDpN35tZd8ArPB3G8VOQDP4g8tVu";
      ...
      const imgPath = path.join(
        tmpDir,
        `myking_${senderID}_${targetID}_${Date.now()}.png`
      );
      ...
      // 👑 Random captions
      const captions = [
        `👑 ${name1} বলছে — এইটা আমার King ${name2}!`,
        `🔥 ${name2} is the true king for ${name1}!`,
        `💪 ${name1} & ${name2} — royal energy!`,
        `👑 ${name1} proudly saying: that's my king ${name2}!`,
        `⚔️ ${name2} always protects ${name1}!`,
        `✨ ${name1} and ${name2} — king level duo!`
      ];
      ...
      console.error("MyKing error:", err);
      message.reply("⚠️ MyKing render failed!");
    }
  }
};
