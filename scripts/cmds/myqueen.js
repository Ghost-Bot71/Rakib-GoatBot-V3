...
module.exports = {
  config: {
    ...
    countDown: 5,
    role: 0,
    shortDescription: "Show your queen",
    category: "fun",
  ...
  onStart: async function ({ message, event, usersData }) {
    ...
    if (!targetID)
      return message.reply("👑 কাউকে reply বা mention করো — এইটা তোমার queen!");
    ...
    try {
      ...
      // background
      const bgURL =
        "https://drive.google.com/uc?export=download&id=1lOwW_SvOsRvPxNmQaX_HwvIpaspE5hoy";
      ...
      const imgPath = path.join(
        tmpDir,
        `myqueen_${senderID}_${targetID}_${Date.now()}.png`
      );
      ...
      // random captions
      const captions = [
        `👑 ${name1} বলছে — এইটা আমার Queen ${name2}!`,
        `💖 ${name2} is the queen of ${name1}'s heart!`,
        `✨ ${name1} proudly presenting his queen ${name2}`,
        `💞 ${name1} & ${name2} — king and queen vibes`,
        `🔥 ${name2} rules the heart of ${name1}`,
        `👑 ${name1} says: My Queen is ${name2}`
      ];
      ...
      console.error("MyQueen error:", err);
      message.reply("⚠️ MyQueen render failed!");
    }
  }
};
