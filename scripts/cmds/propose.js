...
module.exports = {
  ...
  onStart: async function ({ message, event, usersData }) {
    ...
    if (!targetID)
      return message.reply("💍 কাউকে reply বা mention করো proposal দেওয়ার জন্য!");
    ...
    try {
      ...
      // Background image
      const bgURL =
        "https://drive.google.com/uc?export=download&id=1i9PjHDksq3EVSW-_kfK9QxKc0cj4qLVe";
      ...
      const imgPath = path.join(
        tmpDir,
        `propose_${senderID}_${targetID}_${Date.now()}.png`
      );

      await fs.writeFile(imgPath, canvas.toBuffer("image/png"));

      // Random proposal captions
      const captions = [
        `💍 ${name1} finally proposed to ${name2}!`,
        `❤️ ${name1} asked ${name2} — will you be mine forever?`,
        `💘 ${name1} is proposing to ${name2}!`,
        `🥰 ${name1} couldn't hide the feelings anymore for ${name2}!`,
        `💖 ${name1} wants to spend forever with ${name2}!`,
        `✨ Romantic moment! ${name1} proposed to ${name2}!`
      ];
      ...
      message.reply("⚠️ Proposal render failed!");
    }
  }
};
