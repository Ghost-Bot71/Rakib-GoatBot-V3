...
function indexToSuffix(index) {
	...
	let s = "";
	while (index >= 0) {
		s = String.fromCharCode(97 + (index % 26)) + s;
...
function formatMoney(value) {
	...
	const idx = Math.floor((digits - 1) / 3);
	const leadLen = digits - idx * 3;

	const lead = str.slice(0, leadLen);
	const dec = str.slice(leadLen, leadLen + 1);
...
/* =========================
   PARSE AMOUNT (NO k→m BUG)
========================= */
function parseAmount(arg, mode, wallet, bank, loan) {
	if (!arg) return null;
	arg = String(arg).toLowerCase();

	if (arg === "all") {
		if (mode === "wallet") return safeBigInt(wallet);
		if (mode === "bank") return safeBigInt(bank);
		if (mode === "loan") return safeBigInt(loan);
		return null;
	}

	if (/^\d+$/.test(arg)) return safeBigInt(arg);

	const m = arg.match(/^(\d+)(?:\.(\d+))?([a-z]+)$/);
	if (!m) return null;

	const intP = m[1];
	const decP = (m[2] || "").padEnd(3, "0").slice(0, 3);
	const suf = m[3];

	const idx = suffixToIndex(suf);
	if (idx < 1) return null;

	const zeros = "0".repeat(idx * 3);
	return safeBigInt(intP + decP + zeros.slice(decP.length));
}

/* =========================
   AUTO BANK DEPOSIT (150cs)
========================= */
const WALLET_LIMIT = (() => {
	const idx = suffixToIndex("cs");
	return 150n * (1000n ** BigInt(idx));
})();

function applyWalletLimit(wallet, bank) {
	wallet = safeBigInt(wallet);
	bank = safeBigInt(bank);
